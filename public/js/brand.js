...
var brandManager = {
  ...
  validate: function validate() {
    // Clear error styles
    brandManager.brandForm.find('input, select').removeClass('is-invalid'); // Init Validation
      ...
      if ($('#name').val() != brandManager.originalName) {
        ...
        $.ajax({
          ...
          success: function success(data) {
            console.log(data); // If name and brand unique key exists
          ...
          error: function error(xhr, _error) {
            _utils_js__WEBPACK_IMPORTED_MODULE_0__["default"].removeFlash('#' + brandManager.modalId + ' .modal-body');
            _utils_js__WEBPACK_IMPORTED_MODULE_0__["default"].addFlash('danger', '#' + brandManager.modalId + ' .modal-body', 'Erro ao obter informações de nome.');
          }
        });
      } else {
        brandManager.validateLogic();
  ...
  showErrors: function showErrors() {
    ...
    if (!brandManager.isValid) {
      for (var i = brandManager.brandErrorMessageList.length - 1; i >= 0; i--) {
        _utils_js__WEBPACK_IMPORTED_MODULE_0__["default"].addFlash('danger', '#' + brandManager.modalId + ' .modal-body', brandManager.brandErrorMessageList[i]);
      }
    }
  }
};
    ...
    if (this_action == 'edit') {
      ...
      $.get(page + "/loadModal" + "/" + this_id, function (data) {
        ...
        $('#editRecordModal').on('shown.bs.modal', function () {
          ...
          brandManager.init();
          brandManager.modalId = 'editRecordModal'; // Mark as validation screen
    ...
    if (this_action == 'delete') {
      ...
      $.get(page + "/loadModalDelete" + "/" + this_id, function (data) {
        ...
        $('#deleteRecordModal').on('shown.bs.modal', function () {
          ...
          brandManager.init();
          brandManager.modalId = 'deleteRecordModal'; // Mark as validation screen
...
/***/ 7:
/*!*******************************************!*\
  !*** multi ./resources/js/admin/brand.js ***!
  \*******************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! C:\xampp\htdocs\mundovegano\resources\js\admin\brand.js */"./resources/js/admin/brand.js");
